...
final class DinoFighter: CharacterBody2D {
  ...
  override func _ready() {
    ...
    attack.onEnter = { [weak self] phase in
      ...
      case .active:
        setHitActive(true)
        syncFacing(facing, baseOffset: kickOffset)
      case .recovery:
        setHitActive(false)
      }
    }
    attack.onExit = { [weak self] phase in
      if phase == .active { self?.setHitActive(false) }
    }
    attack.onFinish = { [weak self] in self?.setHitActive(false) }
  ...
  private func buildStates() {
    machine.add(State.idle, .init(
      onUpdate: { [weak self] _ in
        guard let self else { return }
        if inputs.down("crouch") { machine.transition(to: State.crouch); return }
        if inputs.pressed("kick") { machine.transition(to: State.attack); beginKick(); return }
        if inputs.down("move_left") != inputs.down("move_right") { machine.transition(to: State.move); return }
        velocity.x = 0
      }
    ))
    ...
    machine.add(State.move, .init(
      onUpdate: { [weak self] _ in
        ...
        if left == right { machine.transition(to: State.idle); return }
    ...
    machine.add(State.hurt, .init(
      onUpdate: { [weak self] _ in self?.velocity.x = 0 }
    ))
  }
  ...
  private func resolveHitsOnce() {
    guard attack.current == .active, didHitThisAttack == false else { return }
  ...
  private func beginKick() {
    attack.begin([.startup(0.08), .active(0.12), .recovery(0.20)])
  }
